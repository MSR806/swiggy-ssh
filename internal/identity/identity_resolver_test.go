...
func TestResolverFoundIdentity(t *testing.T) {
	repo := newTestRepo()
	...
	resolved, err := resolver.ResolveSSHKey(context.Background(), "ssh", key)
	if err != nil {
...
func TestResolverUnknownIdentityCreatesUserAndBinding(t *testing.T) {
	repo := newTestRepo()
	resolver := NewResolver(repo)
	key := newSSHPublicKey(t)
...
func TestResolverRevokedIdentityRejected(t *testing.T) {
	repo := newTestRepo()
	resolver := NewResolver(repo)
	key := newSSHPublicKey(t)
	fingerprint := ssh.FingerprintSHA256(key)
	...
	_, err := resolver.ResolveSSHKey(context.Background(), "ssh", key)
	if !errors.Is(err, ErrSSHIdentityRevoked) {
		t.Fatalf("expected ErrSSHIdentityRevoked, got %v", err)
	}
}

func TestResolverMissingKeyRejected(t *testing.T) {
	repo := newTestRepo()
	resolver := NewResolver(repo)

	_, err := resolver.ResolveSSHKey(context.Background(), "ssh", nil)
...
func TestResolverConcurrentUnknownKeyRaceResolvesSameIdentity(t *testing.T) {
	...
	go func() {
		identity, err := resolver.ResolveSSHKey(context.Background(), "ssh", key)
