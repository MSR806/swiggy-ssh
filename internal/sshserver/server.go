...
type SSHServer struct {
	...
	tracker       *identity.SessionTracker
...
func New(addr, hostKeyPath string, logger *slog.Logger, resolver *identity.Resolver, tracker *identity.SessionTracker, loginCodeSvc auth.LoginCodeService, publicBaseURL string, authSvc *auth.AuthService) *SSHServer {
	return &SSHServer{
		addr:          addr,
...
func (s *SSHServer) handleConn(ctx context.Context, netConn net.Conn, serverConfig *ssh.ServerConfig) {
	...
	if s.resolver != nil && publicKeyAuthorized != "" {
		...
		if parseErr != nil {
			...
			resolved, resolveErr := s.resolver.ResolveSSHKey(ctx, identity.ClientProtocolSSH, publicKey)
			if resolveErr != nil {
	...
	if s.tracker != nil {
		...
		trackedSession, trackErr := s.tracker.StartSession(ctx, identity.TrackSessionInput{
		...
		if trackErr != nil {
			...
			defer func() {
				...
				defer cancel()
				if endErr := s.tracker.EndSession(endCtx, trackedSession.ID); endErr != nil {
